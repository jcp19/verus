...
use syn_verus::{
    Fields, FnArg, FnArgKind, FnMode, ForeignItem, ForeignItemFn, ForeignItemMacro, ForeignItemStatic,
    ...
    Receiver, Signature, Stmt, Type, TraitItem, TraitItemConst, TraitItemMacro, TraitItemMethod, TraitItemType,
...
impl Printer {
    ...
    fn item_fn(&mut self, item: &ItemFn) {
        ...
        // if item.sig.recommends
        
        match &item.sig.ensures {
            Some(ensures) => {
                self.hardbreak();
                self.word("ensures ");
                self.hardbreak();
        ...
        // TODO
        // if item.sig.decreases

        match &item.sig.mode {
            FnMode::Spec(_) | FnMode::SpecChecked(_) if item.block.stmts.is_empty() => {
                self.word(";");
                self.end();
            }
            _ => {
